    ...
    >
      ...
      >
        ...
        <view
          ...
          :key="pIndex"
          :class="['page-line', p.center && 'page-line-center']"
          :style="{
            'font-size': p.isTitle ? `${bookOption.sizeInfo.title}px` : `${bookOption.sizeInfo.p}px`,
            marginTop: !p.isTitle && p.pFirst ? `${bookOption.sizeInfo.margin}px` : '',
            lineHeight: 1.5,
          }"
        >
          <view
            ...
            :style="{
              marginBottom:
                pIndex === getLastTitleIndex(pageTextList[index].content) ? `${bookOption.sizeInfo.margin}px` : '',
            }"
          >
            {{ p.text }}
          </view>
          <text
            v-else
            v-for="(char, ix) in p.text.split('')"
            :key="ix"
            :style="{
              'margin-left': p.pFirst && ix === 0 ? `${bookOption.sizeInfo.p * 2}px` : '0',
            }"
          >
            {{ char }}
          </text>
...
/** 章节阅读器数据 */
const chapterList = ref<ChapterList[]>([]);
/** 页面内容列表 */
const pageTextList = ref<Array<{ title: string; content: PageList }>>([
  {
    title: "", // "page-1",
    content: [],
  },
...
function getPagination(index: number) {
  if (index === 0) {
    if (pageIndex.value === 0) {
      const length = chapterList.value[chapter.value - 1]?.length;
      return `${length}/${length}`;
    } else {
      return `${pageIndex.value}/${pageCount.value}`;
    }
  } else if (index === 1) {
    ...
    if (pageIndex.value === pageCount.value - 1) {
      ...
      return `${pageIndex.value + 2}/${pageCount.value}`;
    }
  }
  return pageCount.value;
}

function getLastTitleIndex(page: PageList) {
  const ix = page.filter((i) => i.isTitle)?.length || 0;
  return ix - 1;
}
...
function updateCurrentInfo(data: bookInfo) {
  if (!(data.bookName && data.author && data.links.length)) {
    showToast("解析失败");
    return;
  }
  // remainOrigins
...
function updateContent() {
  ...
  if (isFirstPage.value) {
    ...
    pageTextList.value[0].content = contents[pageIndex.value - 1];
  }
  // 当前显示的内容 =====================================================================
  pageTextList.value[1].content = contents[pageIndex.value];
  ...
  if (isLastPage.value) {
    ...
    pageTextList.value[2].content = contents[pageIndex.value + 1];
  }
}
...
function loadPrePage(chapter: number, index: number) {
  if (index === 0) {
    ...
    pageTextList.value[0].content = chapterList.value[chapter][lastPage];
  } else if (index === 2) {
    pageTextList.value[2].content = chapterList.value[chapter][0];
  }
}
